...
%% Configure the SMUs
gate.setupSMU('VGT', 'IGT', MAT4200A.consts.SourceType.VOLT, MAT4200A.consts.SourceFunction.STEP);
gate.setStepFunction(0, 5, 1, 0.1);

source.setupSMU('VSRC', 'ISRC', MAT4200A.consts.SourceType.VOLT, MAT4200A.consts.SourceFunction.SWEEP);
...
result = ki4200.makeDependentFrom(source.current_measurement, {source.voltage_measurement, gate.voltage_measurement});
...
for i = 1 : numel(vgt_values)
    ...
    plot(vsrc_values, result{vgt_values(i), :}, 'Color', colors{mod(i-1, numel(colors)) + 1}, 'DisplayName', sprintf('Gate = %.2f V', vgt_values(i)));
end
hold off;
xlabel('Vsource (V)');
ylabel('Isource (A)');
