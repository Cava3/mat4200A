...
%   with actionable error messages for each failure mode.
    ...
    try
        ...
        error('MAT4200A:checkPythonEnv:importFailed', ...
              ['py4200A cannot be imported: %s\n' ...
               'Install it with:  pip install py4200a\n' ...
               'If MATLAB is using the wrong Python, run:\n' ...
               '    pyenv(''Version'', ''/path/to/python3.11'')'], ...
              ME.message);
    end
...
function assertVersion_(pe)
    ...
    if v(1) < 3 || (v(1) == 3 && v(2) < 10)
        error('MAT4200A:checkPythonEnv:pythonTooOld', ...
              ['Python 3.10+ required (found %s).\n' ...
               'Run: pyenv(''Version'', ''/path/to/python3.11'')'], ...
              char(pe.Version));
    end
end
