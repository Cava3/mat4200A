...
if isempty(which('Dependent'))
    warning(['MAT4200A:setup:dependantNotFound ' ...
             'Dependent class is not on the MATLAB path. ' ...
             'makeDependentFrom() will not work until Dependent.m is accessible.']);
else
...
if strcmp(pe.Status, 'NotLoaded')
    warning(['MAT4200A:setup:pythonNotLoaded', ...
             'Python environment is not loaded. MATLAB will load it on the first py.* call. ' ...
             'If you need a specific environment run pyenv(''Version'', ''/path/to/python'') first.']);
else
    ...
    if numel(verParts) < 2 || verParts(1) < 3 || (verParts(1) == 3 && verParts(2) < 10)
        error('MAT4200A:setup:pythonTooOld', ...
              ['Python 3.10 or newer is required (found %s).\n' ...
               ...
               '    pyenv(''Version'', ''C:\\path\\to\\python3.11\\python.exe'')'], ...
              char(pe.Version));
    end
    if verParts(1) == 3 && verParts(2) >= 13
        error('MAT4200A:setup:pythonTooNew', ...
              ['Python %s is not supported by MATLAB R2025b (max: 3.12).\n' ...
               'Install Python 3.12 from https://python.org and point MATLAB to it:\n' ...
               '    pyenv(''Version'', ''C:\\path\\to\\python3.12\\python.exe'')'], ...
              char(pe.Version));
    end
end
...
try
    ...
    if pipStatus == 0
        ...
        error('MAT4200A:setup:notImportable', ...
              ['py4200A is installed (see pip show above) but cannot be imported.\n' ...
    ...
    if strcmpi(strtrim(answer), 'y')
        ...
        if status == 0
            ...
            error('MAT4200A:setup:restartRequired', ...
                  ['py4200A was installed successfully.\n' ...
                   'Please restart MATLAB and run setup again to complete setup.']);
        else
            error('MAT4200A:setup:installFailed', ...
                  'pip install failed:\n%s', out);
        end
    else
        error('MAT4200A:setup:importFailed', ...
              ['py4200A is not installed.\n', ...
